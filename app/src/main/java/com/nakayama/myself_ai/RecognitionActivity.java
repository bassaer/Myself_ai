...
public class RecognitionActivity extends FragmentActivity implements VoiceRecognitionEventListener {
    ...
    public void onRecord(short[] samples) {
        ...
        for (short s : samples) {
            ...
            Double db = Math.log10(Math.pow(s, 2) / Math.pow(Short.MAX_VALUE, 2)) * 10;
            avg += db;
        }
        avg /= samples.length;
     ...
     *
     * @see jp.co.ntt.lab.speechrec.android.service.helper.VoiceRecognitionEventListener#onFinish(java.lang.Exception)
     */
    @Override
